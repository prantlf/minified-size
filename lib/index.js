...
const { gzip: gzipCompress } = require('zlib')
...
async function minifyJavaScript (source, minifier) {
  if (!minifier || minifier === 'esbuild') {
    const { code: minifiedSource } = await transformWithESBuild(source, { minify: true })
...
async function estimateSizes (language, { file, source, size: originalSize, error }, gzip, minifier) {
  ...
  try {
    ...
    const gzippedSize = await estimateGzippedSize(minifiedBuffer, gzip)
